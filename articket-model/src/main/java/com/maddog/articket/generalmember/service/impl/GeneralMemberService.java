...
public class GeneralMemberService {
	...
	public GeneralMember getById(Integer id) {
		return generalMemberDao.findById(id).orElse(null); // 使用 JPA 獲取會員資料
	}

	// 會員修改密碼
	@Transactional
	public void changePassword(Integer memberID, String currentPassword, String newPassword) {
		GeneralMember member = generalMemberDao.findById(memberID).orElseThrow(() -> new RuntimeException("會員不存在"));
	...
	public GeneralMember login(String memberAccount, String password) {
		GeneralMember member = generalMemberDao.findByMemberAccount(memberAccount);
		if (member == null) {
