 ...
 * 				Integer
	 ...
	 * 			ResponseEntity<byte[]>
	 */
	@GetMapping("image/{activityPictureId}")
	public ResponseEntity<byte[]> showImage(@PathVariable("activityPictureId") Integer activityPictureId){
		try {
			byte[] image = activityPictureSvc.getOneActivityPicture(activityPictureId).getActivityPicture();

			return ResponseEntity.ok()
					.header("Content-Type", "image/*")
					.body(image != null ? image : ALT_IMAGE);
		}catch(Exception e) {
			return ResponseEntity.ok()
					.header("Content-Type", "image/*")
					.body(ALT_IMAGE);
